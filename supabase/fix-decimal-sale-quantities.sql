...
DECLARE v_id uuid; v_drawer drawer_balances%ROWTYPE;
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    INSERT INTO drawer_balances(date, shift_id, cash, coin, till) VALUES (p_expense_date, NULL, COALESCE(v_drawer.cash, 0) - p_cash_deducted, COALESCE(v_drawer.coin, 0) - p_coin_deducted, COALESCE(v_drawer.till, 0) - p_till_deducted);
  ELSE
...
DECLARE v_expenses expenses%ROWTYPE;
BEGIN
  SELECT * INTO v_expenses FROM expenses WHERE id = p_expense_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Expense not found'; END IF;
  UPDATE drawer_balances SET cash = cash + COALESCE(v_expenses.cash_deducted, 0), coin = coin + COALESCE(v_expenses.coin_deducted, 0), till = till + COALESCE(v_expenses.till_deducted, 0), updated_at = now() WHERE date = v_expenses.expense_date AND shift_id IS NULL;
...
RETURNS void AS $$
DECLARE v_sale sales%ROWTYPE; v_total numeric;
BEGIN
  SELECT * INTO v_sale FROM sales WHERE id = p_sale_id FOR UPDATE;
  ...
  SET cash = cash + CASE WHEN p_payment_method = 'cash' THEN v_total ELSE 0 END,
      ...
      updated_at = now()
  WHERE date = v_sale.created_at::date AND shift_id IS NOT DISTINCT FROM v_sale.shift_id;
  UPDATE sales SET discount = p_discount, payment_method = p_payment_method, total_amount = v_total, updated_at = now() WHERE id = p_sale_id;
END;
$$ LANGUAGE plpgsql;
